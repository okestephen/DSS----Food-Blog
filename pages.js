...
router.get("/", async (req, res) => {
  try {
    const getRecipes = await db.query(`
      ...
      LIMIT 5;
    `);
    res.render("index.ejs", { featuredRecipes: getRecipes.rows });
  } catch (err) {
...
router.get("/recipe/:id", async (req, res) => {
  try {
    ...
    res.render("recipe.ejs", { recipe: result.rows[0] });
  } catch (err) {
...
router.post("/recipe/:id/delete", ensureAuthenticated, async (req, res) => {
  try {
    ...
    const recipe = result.rows[0];
    if (!recipe || recipe.user_id !== req.session.user.id) return res.status(403).send("Not authorized");
    await db.query("DELETE FROM recipes WHERE recipe_id = $1 AND user_id = $2", [req.params.id, req.session.user.id]);
    res.redirect("/browse");
  } catch (err) {
    console.error(err);
...
router.get("/browse", async (req, res) => {
  try {
    ...
    const result = searchTerm
      ? await db.query(`SELECT recipe_id, title, main_image FROM recipes WHERE title ILIKE $1 OR description ILIKE $1 ORDER BY created_at DESC`, [`%${searchTerm}%`])
      : await db.query(`SELECT recipe_id, title, main_image FROM recipes ORDER BY created_at DESC LIMIT 20`);
    res.render("browse.ejs", { recipes: result.rows, searchTerm });
  } catch (err) {
    console.error(err);
    res.render("browse.ejs", { recipes: [], searchTerm: "", error: "Could not load recipes." });
  }
});

router.get("/profile/:slug", ensureAuthenticated, validateSessionIntegrity, async (req, res) => {
  const { slug } = req.params;
  const result = await db.query("SELECT * FROM users WHERE slug = $1", [slug]);
  if (result.rows.length === 0) return res.status(404).send("User not found");
  const user = result.rows[0];
  if (user.user_id !== req.session.user.id) return res.status(403).send("Access Denied");
